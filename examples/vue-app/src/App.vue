...
export default Vue.extend({
  ...
  methods: {
    ...
    getPublicAddress() {
      ...
      torus?.getPublicAddress({ verifier: this.selectedVerifier, verifierId: this.verifierId } as VerifierArgs).then(this.console).catch(console.error)
    },
    getEncryptionKey() {
      const { web3 } = web3Obj;
      const self = this;
