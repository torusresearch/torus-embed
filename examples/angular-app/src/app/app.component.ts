...
export class AppComponent {
  ...
  async sendEth() {
    try {
      window.torus.web3.eth.sendTransaction(
        { from: this.address, to: this.address, value: window.torus.web3.toWei('0.01') },
        (err, transactionHash) => {
  ...
  async logout() {
    await window.torus
      .logout()
      .then(() => ((this.address = ''), (this.balance = '')))
  ...
  getPublicAddress() {
    window.torus.getPublicAddress({ verifier: 'google', verifierId: 'chai@tor.us' }).then(address => this.publicAddress = address)
  }
}
